...
> **author**: BA Agent
> **created**: 2026-08-07
> **last_updated**: 2026-08-19 (v3.1.56 门店信息展示精简：去营业时间/去联系人，保留联系电话+拨打按钮)
...
| v3.1.53 | 2026-08-19 | 精选直播列表筛选项改为「直播中/直播预告/观看记录」三选项：①精选直播页筛选项由「全部状态/直播中/预告/回放」改为「直播中/直播预告/观看记录」三选项（移除"全部状态"）；②「直播中」选项包含直播中+回放中两种状态（live/replay），「直播预告」=upcoming，「观看记录」=ended；③进入精选直播Tab默认选中「直播中」；④卡片状态标签仍按实际状态显示不变 |
| v3.1.54 | 2026-08-19 | 精选直播操作按钮增强：①「直播预告」筛选下卡片显示【立即预约】按钮（橙色描边），点击变【已预约】（灰色），再点取消，预约状态localStorage持久化；②「观看记录」筛选数据范围扩展为 ended+live，其中 live 状态的直播显示【观看直播】按钮（橙色实心）点击进入直播详情，ended 不显示按钮；③按钮点击不触发卡片跳转（@click.stop） |
| v3.1.55 | 2026-08-19 | 跳转配置去掉项目行业筛选：①运营后台选择商品/直播由「行业→项目→商品/直播」三级级联恢复为「项目→商品/直播」二级级联；②选择项目主页由「行业→项目」恢复为直接选项目；③项目下拉不再 disabled（直接展示全部启用项目）；④租户后台 lockProjectId 模式不受影响 |
| v3.1.56 | 2026-08-19 | 门店信息展示精简：①门店详情页(StoreDetailContent)去掉营业时间展示；②门店详情页去掉联系人展示；③门店详情页保留联系电话并新增「拨打」按钮(tel:链接)；④门店列表卡片(StoreCard)同步去掉营业时间与联系人展示，保留联系电话+拨打按钮；⑤数据字段 business_hours/contact_name 保留，仅移除页面展示 |
