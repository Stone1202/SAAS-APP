...
### ENT-TRD-002 清分明细(SettlementDetail) 🟡

| 字段 | 类型 | 说明 |
|------|------|------|
| settlement_id | String | 清分ID |
| payment_order_id | String | 关联支付单ID |
| settle_target_type | Enum | 分账方类型（供应商/代理/门店/平台） |
| settle_target_id | String | 分账方ID |
| settle_rule_type | Enum | 按比例/按固定金额/按阶梯 |
| settle_amount | Decimal | 清分金额 |
| settle_status | Enum | 待清分/已清分/清分失败 |
| settle_time | Datetime | 清分时间 |
| rule_config_id | String | 清分规则配置ID |
- **依据链**：①FN-TRD-001 平台直连分账清分到各分账方；②BR-TRD-009 清分规则可配置化；③财务域清分明细

### ENT-TRD-003 担保资金(EscrowFund) 🟡

| 字段 | 类型 | 说明 |
|------|------|------|
| escrow_id | String | 担保资金ID |
| payment_order_id | String | 关联支付单ID |
| locked_amount | Decimal | 锁定金额 |
| lock_time | Datetime | 锁定时间 |
| unlock_condition | Enum | 确认收货/超时自动确认 |
| unlock_deadline | Datetime | 解冻截止时间（发货后N天） |
| unlock_time | Datetime | 实际解冻时间 |
| status | Enum | 锁定中/已解冻/已打款 |
- **依据链**：①FN-TRD-002 担保交易资金锁定→解冻→打款；②BR-TRD-002 担保交易资金锁定规则；③五类图12.3状态机担保锁定状态

### ENT-TRD-004 分期还款记录(InstallmentRecord) 🟡

| 字段 | 类型 | 说明 |
|------|------|------|
| installment_id | String | 分期记录ID |
| payment_order_id | String | 关联支付单ID |
| total_periods | Integer | 总期数（3/6/12） |
| current_period | Integer | 当前期数 |
| period_amount | Decimal | 每期还款金额 |
| due_date | Date | 还款截止日期 |
| repay_status | Enum | 待还款/已还款/逾期 |
| repay_time | Datetime | 实际还款时间 |
...
```
支付单(PaymentOrder) 1 ── N 清分明细(SettlementDetail)
支付单(PaymentOrder) 1 ── 1 担保资金(EscrowFund)      -- 仅担保交易模式
支付单(PaymentOrder) 1 ── N 分期还款记录(InstallmentRecord)  -- 仅分期付款模式
```
...
### 17.1 支付单(PaymentOrder)生命周期CRUD-L

| 操作 | 场景 | 操作者 | 前置约束 | 后置影响 | 跨域影响 |
|------|------|--------|----------|----------|----------|
| Create | 客户发起支付 | 系统 | 订单已创建，状态=待付款 | 支付单创建，状态=待支付 | — |
| Read | 支付查询 | 客户/租户 | — | — | — |
| Update | 支付成功 | 支付网关回调 | 金额校验通过 | 状态=已支付/担保锁定 | 订单状态更新、库存扣减 |
| Update | 担保解冻 | 系统/客户确认 | 担保锁定+发货后N天/确认收货 | 状态=已解冻 | 资金打款 |
| Update | 分账触发 | 系统 | 已解冻/已支付+T+售后期 | 状态=已分账 | 清分明细生成 |
| Update | 退款触发 | 系统/售后 | 已分账+售后完成 | 状态=已退款 | 佣金回滚、积分退还、库存回滚 |
| List | 支付单列表 | 租户/财务 | 权限校验 | — | — |
