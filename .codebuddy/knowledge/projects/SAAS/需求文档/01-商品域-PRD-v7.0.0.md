...
| 目标 | 度量指标 | 目标值 |
|------|----------|--------|
| 商品列表查询性能 | P99 延迟 | < 500ms |
| 商品详情查询性能 | P99 延迟 | < 200ms |
...
- **依据链**：①添加、编辑商品.html line 1212 "采购价：如果商品与供应商进行绑定，则此处修改会同步修改供应商处的该商品的采购价"；②商品管理_1.html line 589 "* 采购价" + line 609 "商品状态"区域多SKU场景

### BR-PRD-027 隐藏剩余件数规则 🟡
- **触发条件**：商品勾选"隐藏剩余件数"
- **行为**：勾选后商品详情页、购物车不显示剩余库存数量，仅商家后台可见
- **依据链**：①添加、编辑商品.html 添加商品配置项"隐藏剩余件数"；②商品管理_1.html line 462 数据查询说明"6：商品图片，主图是否上传"隐含商品可见性控制

### BR-PRD-028 按单限购规则 🟡
- **触发条件**：商品配置按单限购
- **行为**：单笔订单限制购买的数量，超过限购数量不允许下单或提示调整
- **依据链**：①添加、编辑商品.html line 1220 "按单限购"；②商品详情页与购物车需展示限购提示（与订单域下单校验联动）

---

## 9. 数据实体

### ENT-PRD-001 品类(Category)

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| id | Long | 是 | 主键 |
| parentId | Long | 否 | 父品类 ID（顶级为空） |
| name | String | 是 | 品类名称，如"服饰配饰""男装" |
| description | String | 否 | 经营说明 |
| status | Enum | 是 | ENABLED / DISABLED（默认 DISABLED） |
| isSpecialQualification | Boolean | 是 | 是否需要特殊资质 |
| qualifications | List | 否 | 关联的特殊许可证列表（共N个） |
| createTime | DateTime | 是 | 创建时间 |
| createUser | String | 是 | 创建人 |
| lastUpdateTime | DateTime | 是 | 最后更新时间 |
...
|------|------|------|------|
| id | Long | 是 | 主键 |
| spuCode | String | 是 | SPU 编码（系统生成） |
| name | String | 是 | 商品名称 |
| categoryId | Long | 是 | 所属品类 ID |
| productType | Enum | 是 | 商品类型（当前仅 PHYSICAL 实物类） |
| mainImages | List | 是 | 商品主图（最多 15 张） |
| introVideo | String | 否 | 讲解视频（最多 1 个） |
| status | Enum | 是 | AUDITING / ON_SALE / AUDIT_REJECTED / OFF_SHELF |
| salesCount | Integer | 是 | 销量 |
| totalStock | Integer | 是 | 总库存（SKU 汇总） |
| purchasePriceRange | String | 是 | 采购价区间 |
| salePriceRange | String | 是 | 售价区间 |
| deliveryMethods | List | 是 | 配送方式（快递/到店自提） |
| freightType | Enum | 是 | 邮费类型（统一邮费/运费到付） |
| freightTemplateId | Long | 否 | 运费模版 ID |
| serviceGuarantees | List | 否 | 服务保障（7天无理由/破碎包退/假一赔三/免除邮费） |
| detailContent | Text | 否 | 商品图文介绍 |
| isHideStock | Boolean | 否 | 是否隐藏剩余件数 |
| perOrderLimit | Integer | 否 | 按单限购数量 |
| supplierId | Long | 否 | 所属供应商 ID |
| createTime | DateTime | 是 | 创建时间 |
| lastUpdateTime | DateTime | 是 | 最后更新时间 |
...
|------|------|------|------|
| id | Long | 是 | 主键 |
| spuId | Long | 是 | 所属 SPU ID |
| skuCode | String | 是 | SKU 编码（人工录入） |
| barcode | String | 否 | 规格条码 |
| specValues | List | 是 | 规格值组合（如：颜色=红色,尺寸=S） |
| specImage | String | 否 | 规格图片（第一个规格可上传） |
| totalStock | Integer | 是 | 总库存 |
| purchasePrice | Decimal | 是 | 采购价 |
| salePrice | Decimal | 是 | 售价 |
| listPrice | Decimal | 否 | 划线价 |
| weight | Decimal | 否 | 重量(KG) |
| unit | String | 否 | 单位 |

### ENT-PRD-004 门店SKU(StoreSKU)

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| id | Long | 是 | 主键 |
| skuId | Long | 是 | 关联 SKU ID |
| storeId | Long | 是 | 门店 ID |
| storeStock | Integer | 是 | 门店库存（不能超过总库存） |
| storePrice | Decimal | 是 | 门店售价 |
| isVisible | Boolean | 是 | 是否门店可见 |

### ENT-PRD-005 商品审核记录(AuditLog)

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| id | Long | 是 | 主键 |
| spuId | Long | 是 | 关联 SPU ID |
| auditor | String | 是 | 审核人 |
| auditTime | DateTime | 是 | 审核时间 |
| auditResult | Enum | 是 | PASSED / REJECTED |
| auditReason | String | 否 | 不通过理由（REJECTED 时必填） |

### ENT-PRD-006 类目参数(CategoryParam)

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| id | Long | 是 | 主键 |
| name | String | 是 | 参数名称，如"外层面料" |
| presetValue | String | 否 | 预设值，如"35%聚酯纤维，37%棉" |
| categoryId | Long | 是 | 所属类目 ID |
| isRequired | Boolean | 是 | 商品发布时该参数是否必填 |
| createTime | DateTime | 是 | 创建时间 |

### ENT-PRD-007 类目规格(CategorySpec)

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| id | Long | 是 | 主键 |
| name | String | 是 | 规格名称，如"颜色""尺寸" |
| presetValue | String | 否 | 预设值，如"红色、蓝色、S、M、L" |
| categoryId | Long | 是 | 所属类目 ID |
| createTime | DateTime | 是 | 创建时间 |

### ENT-PRD-008 采购价同步记录(PriceSyncLog) 🟡
...
| syncDirection | Enum | 是 | TENANT_TO_SUPPLIER / SUPPLIER_TO_TENANT |
| oldPrice | Decimal | 是 | 变更前采购价 |
| newPrice | Decimal | 是 | 变更后采购价 |
| operator | String | 是 | 操作人/系统 |
| syncTime | DateTime | 是 | 同步时间 |
| source | String | 是 | 变更来源（商品管理/供应商商品） |
- **依据链**：①添加、编辑商品.html line 1212 采购价双向同步说明；②供应商商品管理 BR-PRD-015 反向同步；③PRD-ISSUE-010 缺少变更日志问题

### 实体关系

```
品类(Category) ──1:N──→ 子品类(Category)
    │
    ├──1:N──→ 类目参数(CategoryParam)
    ├──1:N──→ 类目规格(CategorySpec)
    │
商品(SPU) ──N:1──→ 品类(Category)
    │
    ├──1:N──→ SKU
    ├──1:N──→ 商品图片(ProductImage)
    ├──1:1──→ 商品视频(ProductVideo)
    ├──1:N──→ 商品参数(ProductParam)
    ├──1:N──→ 商品审核记录(AuditLog)
    │
SKU ──N:1──→ 商品(SPU)
    │
    ├──1:N──→ 门店SKU(StoreSKU) ──N:1──→ 门店(Store)
    ├──1:N──→ 直播SKU(LiveSKU) ──N:1──→ 直播间(LiveRoom)
    ├──N:1──→ 供应商(Supplier)
    └──1:N──→ 采购价同步记录(PriceSyncLog)
```
...
- **依据链SC-PRD-007**：①商品管理_1.html line 774 "编辑商品"仅审核中/审核不通过/已下架状态显示；②BR-PRD-011 在售中状态规则
...
| MET-PRD-005 | 商品列表查询P99 | 商品列表查询接口P99延迟 | 系统监控 |
| MET-PRD-006 | 商品详情查询P99 | 商品详情查询接口P99延迟 | 系统监控 |
...
| 场景编号 | 场景名称 | 场景描述 | 依据链 |
|----------|----------|----------|--------|
| ISC-PRD-001 🟡 | 商品复制后独立审核 | 复制的商品进入审核流程，与原商品状态独立。原商品在售不影响复制商品的审核 | ①商品管理_1.html line 549/557 "复制"按钮在在售中/审核中/审核不通过/已下架状态均可用；②BR-PRD-010 复制的商品默认审核中 |
...
| ISC-PRD-005 🟡 | 采购价区间反映多SKU采购价差异 | 采购价区间格式（32.00~123.00）源于同一商品不同SKU的不同采购价 | ①添加、编辑商品.html 采购价格字段支持区间；②商品管理_1.html line 589 "* 采购价"显示为区间 |
| ISC-PRD-006 🟡 | 规格图片仅第一个规格可上传 | 仅第一个规格支持上传图片用于前端规格列表缩略图显示 | ①BR-PRD-009 规格图片规则；②添加、编辑商品.html 规格配置区域 |
| ISC-PRD-007 🟡 | 商品列表数据查询说明6条规则 | 创建时间定义（手动添加/导入/采购首次进入）、商品编号=SPU编码、规格编号=SKU人工录入、价格区间查询SKU价格、商品详情是否有内容、商品图片主图是否上传 | ①商品管理_1.html line 462 数据查询说明6条；②筛选字段清单交叉印证 |
...
| EXP-PRD-006 | 在售商品尝试编辑 | 商品状态==在售中，点击编辑 | 编辑按钮不可用/置灰 | 🟡商品管理_1.html line 774 编辑按钮状态 + BR-PRD-011 |
...
| EXP-PRD-011 🟡 | 采购价同步失败（供应商离线/网络异常） | 商品修改采购价但供应商系统不可达 | 同步失败需重试/告警，本地采购价已变更 | 🟡采购价双向同步机制隐含异常处理需求；依据：①ENT-PRD-008 PriceSyncLog记录同步状态；②PRD-ISSUE-010 变更日志缺失问题 |
...
### 17.1 商品(SPU)生命周期CRUD-L

| 操作 | 场景 | 操作者 | 前置约束 | 后置影响 | 跨域影响 |
|------|------|--------|----------|----------|----------|
| Create | 商品发布 | 租户 | 类目已选择，必填字段已填 | 状态=审核中，生成SKU/审核记录 | — |
| Read | 商品列表查询 | 租户/运营 | 权限校验 | — | — |
| Read | 商品详情查看 | 租户/客户 | — | — | APP端展示 |
| Update | 商品编辑 | 租户 | 状态≠在售中 | 状态=审核中（重新审核） | 影响订单域快照（新订单读取新价格） |
| Update | 商品审核 | 审核员 | 状态=审核中 | 状态=在售中/审核不通过 | APP端可见性变化 |
| Update | 商品上下架 | 租户 | 状态=在售中/已下架 | 状态变更 | 订单域可下单性、直播域可见性 |
| Update | 采购价修改 | 租户/供应商 | 商品已绑定供应商 | 采购价变更+同步日志 | 财务域佣金重算、供应商商品同步 |
| Delete | 商品删除 | 租户 | 状态≠在售中 | 商品软删除 | — |
| List | 商品列表（按商品/按SKU） | 租户 | 权限校验 | 支持筛选/状态Tab/视图切换 | — |
...
| GAP-PRD-003 | 采购价双向同步缺少变更日志（PRD-ISSUE-010），同步失败/异常无法追溯 | 🟡 中 | 新增ENT-PRD-008 PriceSyncLog（V7.0.0已补充） |
...
|--------------|----------|-------|-------|-------|----------|
| BR-PRD-026 | 采购价区间规则 | 添加、编辑商品.html line 1212 | 商品管理_1.html line 589 | — | ✅ 通过（2处） |
| BR-PRD-027 | 隐藏剩余件数规则 | 添加、编辑商品.html 配置项 | 商品管理_1.html line 462 | — | ✅ 通过（2处） |
| BR-PRD-028 | 按单限购规则 | 添加、编辑商品.html line 1220 | 订单域下单校验联动 | — | ✅ 通过（2处） |
| ENT-PRD-008 | 采购价同步记录 | 添加、编辑商品.html line 1212 | 供应商商品管理 BR-PRD-015 | PRD-ISSUE-010 | ✅ 通过（3处） |
| SC-PRD-007 | 在售商品编辑限制 | 商品管理_1.html line 774 | BR-PRD-011 | — | ✅ 通过（2处） |
| ISC-PRD-001 | 商品复制后独立审核 | 商品管理_1.html line 549/557 | BR-PRD-010 | — | ✅ 通过（2处） |
...
| ISC-PRD-005 | 采购价区间反映多SKU差异 | 添加、编辑商品.html 采购价格 | 商品管理_1.html line 589 | — | ✅ 通过（2处） |
| ISC-PRD-006 | 规格图片仅第一个规格 | BR-PRD-009 | 添加、编辑商品.html 规格配置 | — | ✅ 通过（2处） |
| ISC-PRD-007 | 商品列表数据查询6条规则 | 商品管理_1.html line 462 | 筛选字段清单 | — | ✅ 通过（2处） |
| ISC-PRD-008 | 门店商品下单状态实时校验 | BR-PRD-018 | 门店商品.html 流程说明 | — | ✅ 通过（2处） |
| EXP-PRD-011 | 采购价同步失败 | ENT-PRD-008 PriceSyncLog | PRD-ISSUE-010 | — | ✅ 通过（2处） |
