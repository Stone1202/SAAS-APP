...
- **依据链**：①订单整体流程说明.html line 633 "注意：订单数据中，需要独立记录每种优惠形式的总金额，以及优惠总金额，以便于订单逆向操作的退优惠券的逻辑处理"；②ENT-ORD-001 含full_reduction_amount/discount_coupon_amount/red_envelope_amount/points_deduction_amount独立字段

### BR-ORD-012 优惠叠加规则 🟡
- 触发条件：用户领取多种优惠形式
- 行为：用户领取的所有优惠形式的数据，将允许叠加的组合出来，有几种代表用户可以选择几种；极端下用户可以选择三种优惠形式，或者最少只有1种选择入口
- **依据链**：①订单整体流程说明.html line 625 "用户领取的所有优惠形式的数据，将允许叠加的组合出来"；②ENT-ORD-001 优惠金额独立字段（满减+折扣+红包+积分可叠加）

### BR-ORD-013 优惠领取校验规则 🟡
- 触发条件：用户领取优惠券/折扣券/现金红包
- 行为：校验门槛（是否满足门槛）、领取上限（是否达到领取上限）、特定身份（是否存在特定身份限制+是否满足特定身份）
- **依据链**：①订单整体流程说明.html line 334-495 门槛/上限/身份校验流程；②营销域优惠券管理

### BR-ORD-014 优惠匹配最优规则 🟡
- 触发条件：用户有多张优惠券/折扣券/现金红包
- 行为：系统匹配最优优惠券/折扣券/现金红包供用户选择
- **依据链**：①订单整体流程说明.html line 315 "匹配最优优惠券"、line 546 "匹配最优折扣券"、line 579 "匹配最优现金红包"；②营销域优惠券推荐逻辑

### BR-ORD-015 提货单与提货记录关系规则 🟡
- 触发条件：自提订单提货
- 行为：订单1~N提货单，提货单1~N提货记录；提货记录实际提货数量==提货单的数量时，变更提货单状态==已提货
- **依据链**：①自提订单整体流程说明.html line 63 "订单1~N提货单/提货单1~N提货记录/提货记录实际提货数量==提货单的数量，变更提货单==已提货"；②ENT-ORD-004/005 提货单与提货记录实体关系
...
| original_price | Decimal | 原单价（商品快照） |
| quantity | Integer | 数量 |
...
### ENT-ORD-006 订单地址变更记录(AddressChangeLog) 🟡

| 字段 | 类型 | 说明 |
|------|------|------|
| log_id | String | 日志ID |
| order_id | String | 订单编号 |
| old_address | String | 变更前地址 |
| new_address | String | 变更后地址 |
...
订单(Order) 1 ── N 订单地址变更记录(AddressChangeLog)  🟡
...
- **依据链SC-ORD-007**：①订单整体流程说明.html line 625 优惠叠加组合；②BR-ORD-011 优惠金额独立记录
...
- **依据链**：①自提订单整体流程说明.html line 55 "扫码买家任一个提货码，都会反查订单对应的这一组提货码的商品"；②BR-ORD-007 提货单生成规则
...
| 场景编号 | 场景名称 | 场景描述 | 依据链 |
|----------|----------|----------|--------|
| ISC-ORD-001 🟡 | 优惠叠加组合选择 | 用户领取的多种优惠形式允许叠加组合，极端下可选择三种，最少一种 | 订单整体流程说明.html line 625 + BR-ORD-012 |
| ISC-ORD-002 🟡 | 优惠金额逆向退券处理 | 订单逆向操作时，根据独立记录的每种优惠形式总金额退回优惠券/红包/积分 | 订单整体流程说明.html line 633 + ENT-ORD-001 优惠金额独立字段 |
| ISC-ORD-003 🟡 | 优惠领取校验三重校验 | 领取优惠时校验门槛、领取上限、特定身份三重条件 | 订单整体流程说明.html line 334-495 + BR-ORD-013 |
| ISC-ORD-004 🟡 | 提货码反查提货单组 | 店员扫码任一提货码，系统反查订单对应的一组提货码商品 | 自提订单整体流程说明.html line 55 + BR-ORD-007 |
| ISC-ORD-005 🟡 | 提货人/下单人/点货人三方记录 | 提货记录记录三方信息用于追踪提货过程 | 自提订单整体流程说明.html line 55 + ENT-ORD-005 |
| ISC-ORD-006 🟡 | 首次无收货地址时添加并设为默认 | 创建订单读取默认收货地址，无则跳转地址管理添加首个并设默认 | 订单整体流程说明.html line 245-288 |
| ISC-ORD-007 🟡 | 库存逻辑检查 | 立即购买时读取门店通用配置+库存逻辑检查 | 订单整体流程说明.html line 780-796 |
| ISC-ORD-008 🟡 | 订单明细商品快照信息 | 订单明细中商品名称标注"【订单明细商品快照信息】"，独立于商品域实时数据 | 订单详情_待付款.html line 383 + 商品域复用边界商品快照 |
...
| EXP-ORD-005 | 优惠不满足门槛 | 领取的优惠券门槛>订单金额 | 不允许使用该优惠 | 🟡订单整体流程说明.html line 342 门槛校验 |
| EXP-ORD-006 | 优惠达到领取上限 | 用户已领取该优惠达上限 | 不允许再次领取 | 🟡订单整体流程说明.html line 368 上限校验 |
| EXP-ORD-007 | 优惠特定身份不满足 | 优惠有特定身份限制，用户不满足 | 不允许使用该优惠 | 🟡订单整体流程说明.html line 392 身份校验 |
| EXP-ORD-008 | 自提订单无包裹数据 | 发货方式==自提 | 不生成包裹数据，生成提货单 | 📗自提订单整体流程说明.html line 55 |
| EXP-ORD-009 | 提货码错误/无效 | 店员扫码非该订单提货码 | 反查失败，提示无效码 | 🟡自提订单整体流程说明.html line 55 反查逻辑 |
| EXP-ORD-010 🟡 | 无收货地址下单 | 创建订单时无默认收货地址 | 跳转地址管理，添加首个并设为默认 | 🟡订单整体流程说明.html line 245-288 |
| EXP-ORD-011 🟡 | 库存不足下单 | 立即购买时库存逻辑检查不通过 | 下单失败/提示库存不足 | 🟡订单整体流程说明.html line 796 库存逻辑检查 |
...
### 17.1 订单(Order)生命周期CRUD-L

| 操作 | 场景 | 操作者 | 前置约束 | 后置影响 | 跨域影响 |
|------|------|--------|----------|----------|----------|
| Create | 客户下单 | 客户(APP) | 商品在售+库存充足+收货地址存在 | 状态=待付款，生成订单明细+商品快照 | 库存锁定、营销资源锁定 |
| Read | 订单详情查看 | 租户/客户 | — | — | — |
| Update | 支付成功 | 系统/支付回调 | 支付金额=应付 | 状态=待发货/待自提 | 库存扣减、生成提货码(自提) |
| Update | 发货 | 租户 | 状态=待发货 | 状态=已发货/待发货含部分 | 物流运单创建、消息推送 |
| Update | 签收/确认收货 | 客户/系统 | 状态=已发货 | 状态=已完成 | 佣金结算触发 |
| Update | 自提核销 | 店员 | 状态=待自提 | 状态=已完成/待自提部分 | 提货记录生成 |
| Update | 取消/超时关闭 | 客户/系统 | 状态=待付款 | 状态=已关闭 | 库存释放 |
| Update | 发起售后 | 客户 | 状态≥待发货 | 状态=售后中(挂起) | 售后单创建 |
| Update | 售后完成恢复 | 系统 | 售后单完成/关闭 | 恢复原状态 | 佣金回滚/积分退还 |
| Update | 修改地址 | 租户 | 状态≤待发货 | 地址变更+日志 | — |
| Delete | 删除订单 | 客户(APP) | 状态=已完成/已取消 | 软删除 | — |
| List | 订单列表 | 租户/客户 | 权限校验 | 支持筛选/状态Tab/搜索 | — |
...
| **客户(APP)** | 取消/立即支付 | 查看详情/申请退款 | 确认收货/查看物流/申请售后 | 查看自提码/申请售后 | 评价/删除/申请售后 | 删除 | — |
| **店员/店长** | — | — | — | 扫码核销 | — | — | — |
...
| GAP-ORD-004 | 收货地址修改历史版本未明确保留（ORD-ISSUE-005） | 🟢 低 | V7.0.0已补充ENT-ORD-006 AddressChangeLog |
...
|--------------|----------|-------|-------|-------|----------|
| BR-ORD-011 | 优惠金额独立记录 | 订单整体流程说明.html line 633 | ENT-ORD-001 优惠金额字段 | — | ✅ 通过（2处） |
| BR-ORD-012 | 优惠叠加规则 | 订单整体流程说明.html line 625 | ENT-ORD-001 优惠金额字段 | — | ✅ 通过（2处） |
| BR-ORD-013 | 优惠领取校验 | 订单整体流程说明.html line 334-495 | 营销域优惠券管理 | — | ✅ 通过（2处） |
| BR-ORD-014 | 优惠匹配最优 | 订单整体流程说明.html line 315/546/579 | 营销域推荐逻辑 | — | ✅ 通过（2处） |
| BR-ORD-015 | 提货单与提货记录关系 | 自提订单整体流程说明.html line 63 | ENT-ORD-004/005 实体关系 | — | ✅ 通过（2处） |
| ENT-ORD-006 | 订单地址变更记录 | ORD-ISSUE-005 | 修改地址(快递/自提).html | — | ✅ 通过（2处） |
| SC-ORD-007 | 多优惠叠加下单 | 订单整体流程说明.html line 625 | BR-ORD-011 | — | ✅ 通过（2处） |
| ISC-ORD-001 | 优惠叠加组合选择 | 订单整体流程说明.html line 625 | BR-ORD-012 | — | ✅ 通过（2处） |
| ISC-ORD-002 | 优惠金额逆向退券 | 订单整体流程说明.html line 633 | ENT-ORD-001 优惠金额字段 | — | ✅ 通过（2处） |
| ISC-ORD-003 | 优惠领取三重校验 | 订单整体流程说明.html line 334-495 | BR-ORD-013 | — | ✅ 通过（2处） |
| ISC-ORD-004 | 提货码反查提货单组 | 自提订单整体流程说明.html line 55 | BR-ORD-007 | — | ✅ 通过（2处） |
| ISC-ORD-005 | 三方提货记录 | 自提订单整体流程说明.html line 55 | ENT-ORD-005 | — | ✅ 通过（2处） |
| ISC-ORD-006 | 首次无地址添加默认 | 订单整体流程说明.html line 245-288 | 地址管理 | — | ✅ 通过（2处） |
| ISC-ORD-007 | 库存逻辑检查 | 订单整体流程说明.html line 780-796 | 库存域库存扣减 | — | ✅ 通过（2处） |
| ISC-ORD-008 | 订单明细商品快照 | 订单详情_待付款.html line 383 | 商品域复用边界商品快照 | — | ✅ 通过（2处） |
| EXP-ORD-005 | 优惠不满足门槛 | 订单整体流程说明.html line 342 | BR-ORD-013 | — | ✅ 通过（2处） |
| EXP-ORD-006 | 优惠达到领取上限 | 订单整体流程说明.html line 368 | BR-ORD-013 | — | ✅ 通过（2处） |
| EXP-ORD-007 | 优惠特定身份不满足 | 订单整体流程说明.html line 392 | BR-ORD-013 | — | ✅ 通过（2处） |
| EXP-ORD-009 | 提货码错误无效 | 自提订单整体流程说明.html line 55 | BR-ORD-007 | — | ✅ 通过（2处） |
| EXP-ORD-010 | 无收货地址下单 | 订单整体流程说明.html line 245-288 | ISC-ORD-006 | — | ✅ 通过（2处） |
| EXP-ORD-011 | 库存不足下单 | 订单整体流程说明.html line 796 | 库存域库存扣减 | — | ✅ 通过（2处） |
