# 售后域 产品需求文档 PRD V7.0.0

> **文档类型**：业务域产品需求文档（PRD）
> **所属系统**：九天 SAAS 电商平台
> **业务域**：售后域（D04）
> **文档版本**：V7.0.0（基于 V1.0.3 执行 V7.0.0 场景深度还原增强）
> **编制日期**：2026-07-21
...
- **依据链**：①售后单整体流程说明.html line 783 "同意后，会生成该售后单1~1支付单，使用此支付单单号，退款总金额、应退总金额等关键信息，向三方渠道提交退款订单"；②交易域ENT-TRD-001 支付单关联；③BR-AFS-006 退款方式规则

### BR-AFS-008 订单多售后单规则 🟡
- 触发条件：买家在订单发起售后
- 行为：可针对全部或部分商品发起售后申请，一个订单可产生1~N个售后单
- **依据链**：①售后单整体流程说明.html line 783 "买家在订单发起售后，会针对全部，或者部分商品进行发起售后申请，此刻该订单1~N个售后单"；②BR-AFS-004 售后完成联动订单规则（全部/部分）

### BR-AFS-009 退货退款售后包裹规则 🟡
- 触发条件：退货退款同意后买家寄回
- 行为：售后单1~N寄回包裹信息，与订单的发货包裹数据区分为【销售包裹】和【售后包裹】两类
- **依据链**：①售后单整体流程说明.html line 783 "售后单1~N寄回包裹信息，与订单的发后包裹数据表这里会出现两类包裹类型...暂定为【销售包裹】、【售后包裹】"；②ENT-ORD-003 发货包裹记录；③ENT-AFS-003 退货信息

### BR-AFS-010 退货退款协商场景规则 🟡
- 触发条件：退货退款商家与买家私下协商
- 行为：支持多种协商结果：①同意寄回验收通过退款完成；②同意但验收不通过商家寄给买家关闭售后；③同意但部分验收不通过协商部分退款完成；④不一致但商家愿意补充退款一部分完成
- **依据链**：①售后单整体流程说明.html line 783 "2a/2a-01/2a-02/2b"协商场景；②售后管理.html 多状态展示；③退货退款状态机

### BR-AFS-011 商家拒绝后重新发起规则 🟡
- 触发条件：商家拒绝售后申请
- 行为：买家只能关闭后重新发起，后续需控制频次
- **依据链**：①售后单整体流程说明.html line 783 "如果商家拒绝，那么买家只能关闭，重新发起，这部分后续再控制频次"；②售后管理.html 商家已拒绝状态→售后关闭

### BR-AFS-012 退货退款不支持部分寄回但底层需支持 🟡
- 触发条件：退货退款寄回
- 行为：当前功能暂不支持部分寄回，但底层需要支持（售后单1~N寄回包裹）
- **依据链**：①售后单整体流程说明.html line 783 "这里功能暂不支持部分寄回但是底层需要支持"；②BR-AFS-009 售后包裹1~N关系
...
### ENT-AFS-004 售后包裹(AfterSalePackage) 🟡

| 字段 | 类型 | 说明 |
|------|------|------|
| package_id | String | 售后包裹ID |
| after_sale_id | String | 关联售后单ID |
| logistics_no | String | 寄回物流单号 |
| return_items | Array | 寄回商品明细及数量 |
| package_type | Enum | 售后包裹（区分销售包裹） |
| receive_status | Enum | 未签收/已签收/已拒签 |
- **依据链**：①售后单整体流程说明.html line 783 "售后单1~N寄回包裹信息...两类包裹类型【销售包裹】、【售后包裹】"；②BR-AFS-009 售后包裹规则；③ENT-ORD-003 销售包裹区分

### ENT-AFS-005 售后退款单(RefundOrder) 🟡

| 字段 | 类型 | 说明 |
|------|------|------|
| refund_order_id | String | 退款单ID |
| after_sale_id | String | 关联售后单ID（1~1） |
| payment_order_id | String | 关联支付单ID |
| refund_amount | Decimal | 退款总金额 |
| should_refund_amount | Decimal | 应退总金额 |
| refund_method | Enum | 原路退款/线下退款/超时自动退款 |
| refund_status | Enum | 待退款/退款中/退款成功/退款失败 |
| refund_time | Datetime | 退款时间 |
| third_party流水号 | String | 三方渠道流水号 |
- **依据链**：①售后单整体流程说明.html line 783 "售后单1~1支付单，使用此支付单单号，退款总金额、应退总金额等关键信息，向三方渠道提交退款订单，并且返回流水"；②BR-AFS-007 售后单与支付单关系；③交易域API-TRD-003 退款接口

### ENT-AFS-006 协商记录(NegotiationRecord) 🟡

| 字段 | 类型 | 说明 |
|------|------|------|
| negotiation_id | String | 协商记录ID |
| after_sale_id | String | 关联售后单ID |
| negotiation_type | Enum | 验收不通过/部分验收不通过/商家补充/协商部分退款 |
| negotiation_content | String | 协商内容 |
| refund_adjust_amount | Decimal | 协商调整退款金额 |
| operator | String | 操作人 |
| negotiation_time | Datetime | 协商时间 |
- **依据链**：①售后单整体流程说明.html line 783 "2a-01/2a-02/2b"协商场景；②BR-AFS-010 退货退款协商场景规则；③售后管理.html 多状态展示

### 实体关系

```
订单(Order) 1 ── N 售后单(AfterSales)
售后单(AfterSales) 1 ── N 维权记录(DisputeRecord)
售后单(AfterSales) 1 ── 1 退货信息(ReturnInfo)      -- 仅退货退款
售后单(AfterSales) 1 ── N 售后包裹(AfterSalePackage)  -- 仅退货退款 🟡
售后单(AfterSales) 1 ── 1 售后退款单(RefundOrder)    -- 仅退款/退货退款 🟡
售后单(AfterSales) 1 ── N 协商记录(NegotiationRecord) -- 退货退款协商 🟡
售后退款单(RefundOrder) N ── 1 支付单(PaymentOrder)  -- 关联交易域
```
...
- **依据链SC-AFS-006**：①售后单整体流程说明.html line 783 "2a-02：同意，但是部分验收不通过，又一次协商，这时候退款一部分金额，售后完成"；②BR-AFS-010 协商场景规则
...
- **依据链**：①售后单整体流程说明.html line 783 "同意后，会生成该售后单1~1支付单"；②BR-AFS-007 售后单与支付单关系
...
sequenceDiagram
    ...
    S->>P: POST /api/v1/payment/refund（退款请求，含支付单单号+退款金额）
    P->>P: 处理退款
    P-->>S: 退款回调（含三方流水号）
    S->>S: 更新售后单状态(售后完成)
...
| 场景编号 | 场景名称 | 场景描述 | 依据链 |
|----------|----------|----------|--------|
| ISC-AFS-001 🟡 | 售后单1~1退款单关联支付单 | 仅退款/退货退款同意后生成退款单，1~1关联支付单，向三方渠道提交退款并返回流水 | 售后单整体流程说明.html line 783 + BR-AFS-007 + ENT-AFS-005 |
| ISC-AFS-002 🟡 | 订单1~N售后单 | 一个订单可产生1~N个售后单，针对全部或部分商品发起 | 售后单整体流程说明.html line 783 + BR-AFS-008 |
| ISC-AFS-003 🟡 | 售后包裹与销售包裹区分 | 退货退款寄回产生售后包裹，与订单发货包裹区分为【销售包裹】和【售后包裹】两类 | 售后单整体流程说明.html line 783 + BR-AFS-009 + ENT-AFS-004 |
| ISC-AFS-004 🟡 | 退货退款协商部分退款 | 商家同意后验收部分不通过，协商退款一部分金额后售后完成 | 售后单整体流程说明.html line 783 "2a-02" + BR-AFS-010 + ENT-AFS-006 |
| ISC-AFS-005 🟡 | 退货退款验收不通过商家寄回关闭 | 商家同意后验收不通过，商家寄给买家，关闭售后 | 售后单整体流程说明.html line 783 "2a-01" + BR-AFS-010 |
| ISC-AFS-006 🟡 | 退货退款商家补充部分退款 | 不一致但商家愿意补充，退款一部分金额售后完成 | 售后单整体流程说明.html line 783 "2b" + BR-AFS-010 |
| ISC-AFS-007 🟡 | 商家拒绝后买家重新发起 | 商家拒绝后买家只能关闭后重新发起，后续需控制频次 | 售后单整体流程说明.html line 783 + BR-AFS-011 |
| ISC-AFS-008 🟡 | 退货退款底层支持部分寄回 | 当前功能不支持部分寄回但底层需支持（售后单1~N寄回包裹） | 售后单整体流程说明.html line 783 + BR-AFS-012 |
...
### 17.1 售后单(AfterSales)生命周期CRUD-L

| 操作 | 场景 | 操作者 | 前置约束 | 后置影响 | 跨域影响 |
|------|------|--------|----------|----------|----------|
| Create | 客户发起维权 | 客户 | 订单已付款(仅退款)/已签收(退货退款) | 状态=待商家处理，订单挂起 | 订单域挂起 |
| Read | 售后详情查看 | 客户/商家 | — | — | — |
| Update | 商家同意 | 商家/系统(超时) | 状态=待商家处理 | 状态=商家已同意，生成退款单 | 交易域退款单关联 |
| Update | 商家拒绝 | 商家 | 状态=待商家处理 | 状态=商家已拒绝 | — |
| Update | 买家寄回 | 买家 | 状态=商家已同意(退货退款) | 状态=待商家签收，生成售后包裹 | — |
| Update | 商家签收/拒签 | 商家/系统(超时) | 状态=待商家签收 | 状态=待商家退款/商家寄回 | — |
| Update | 退款 | 商家/系统(超时) | 状态=待商家退款 | 状态=售后完成，退款单执行 | 交易域退款、财务域佣金回滚、营销域积分退还 |
| Update | 买家取消/超时关闭 | 买家/系统 | 状态=待商家处理/商家已同意 | 状态=售后关闭 | 订单域恢复 |
| List | 售后列表 | 商家 | 权限校验 | 支持状态Tab筛选 | — |
...
|--------------|----------|-------|-------|-------|----------|
| BR-AFS-007 | 售后单与支付单关系 | 售后单整体流程说明.html line 783 | 交易域ENT-TRD-001 支付单 | BR-AFS-006 退款方式 | ✅ 通过（3处） |
| BR-AFS-008 | 订单多售后单 | 售后单整体流程说明.html line 783 | BR-AFS-004 售后完成联动 | — | ✅ 通过（2处） |
| BR-AFS-009 | 售后包裹与销售包裹区分 | 售后单整体流程说明.html line 783 | ENT-ORD-003 销售包裹 | ENT-AFS-003 退货信息 | ✅ 通过（3处） |
| BR-AFS-010 | 退货退款协商场景 | 售后单整体流程说明.html line 783 | 售后管理.html 多状态 | 退货退款状态机 | ✅ 通过（3处） |
| BR-AFS-011 | 商家拒绝后重新发起 | 售后单整体流程说明.html line 783 | 售后管理.html 商家已拒绝→售后关闭 | — | ✅ 通过（2处） |
| BR-AFS-012 | 退货退款底层支持部分寄回 | 售后单整体流程说明.html line 783 | BR-AFS-009 售后包裹1~N | — | ✅ 通过（2处） |
| ENT-AFS-004 | 售后包裹 | 售后单整体流程说明.html line 783 | BR-AFS-009 售后包裹规则 | ENT-ORD-003 销售包裹区分 | ✅ 通过（3处） |
| ENT-AFS-005 | 售后退款单 | 售后单整体流程说明.html line 783 | BR-AFS-007 售后单与支付单关系 | 交易域API-TRD-003 退款接口 | ✅ 通过（3处） |
| ENT-AFS-006 | 协商记录 | 售后单整体流程说明.html line 783 | BR-AFS-010 协商场景规则 | 售后管理.html 多状态 | ✅ 通过（3处） |
| SC-AFS-006 | 退货退款协商部分退款 | 售后单整体流程说明.html line 783 "2a-02" | BR-AFS-010 协商场景 | — | ✅ 通过（2处） |
| ISC-AFS-001 | 售后单1~1退款单关联支付单 | 售后单整体流程说明.html line 783 | BR-AFS-007 | ENT-AFS-005 | ✅ 通过（3处） |
| ISC-AFS-002 | 订单1~N售后单 | 售后单整体流程说明.html line 783 | BR-AFS-008 | — | ✅ 通过（2处） |
| ISC-AFS-003 | 售后包裹与销售包裹区分 | 售后单整体流程说明.html line 783 | BR-AFS-009 | ENT-AFS-004 | ✅ 通过（3处） |
| ISC-AFS-004 | 退货退款协商部分退款 | 售后单整体流程说明.html line 783 "2a-02" | BR-AFS-010 | ENT-AFS-006 | ✅ 通过（3处） |
| ISC-AFS-005 | 退货退款验收不通过商家寄回关闭 | 售后单整体流程说明.html line 783 "2a-01" | BR-AFS-010 | 退货退款状态机拒签路径 | ✅ 通过（3处） |
| ISC-AFS-006 | 退货退款商家补充部分退款 | 售后单整体流程说明.html line 783 "2b" | BR-AFS-010 | — | ✅ 通过（2处） |
| ISC-AFS-007 | 商家拒绝后买家重新发起 | 售后单整体流程说明.html line 783 | BR-AFS-011 | — | ✅ 通过（2处） |
| ISC-AFS-008 | 退货退款底层支持部分寄回 | 售后单整体流程说明.html line 783 | BR-AFS-012 | — | ✅ 通过（2处） |
