...
> **V1.4.0三阶段思考（PO规划必须遵循）**：
> PO不能跳过业务还原直接做产品架构——系统划分是分析结果不是前提。
> - po-2.5 业务还原：还原真实业务全貌(角色/部门/关键动作/信息流/状态机) + 业务闭环验证(7要素) + 行业对标 + 阶段1扫盲(行业专家+NFR技术扫盲)
> - po-3 系统划分：基于业务还原+商业模式(B2C/B2B/F2B2C等)划分业务系统 + 匹配行业系统专家(expert-registry.yml) + 系统闭环验证(9要素) + 阶段2扫盲
> - po-3.5 终端划分：基于系统划分确定终端 + 终端视角泳道图 + 跨端流转路径 + 阶段3扫盲
> - po-4 Backlog优先级：基于系统×终端矩阵排序，跨端功能作为整体评估(不拆散到不同Sprint)
