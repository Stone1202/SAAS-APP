...
30. **⚠️ 多专家联合盲区审查（BA-30，V5.4.0新增）**：BA完成§1-§6产物后，PM启动§6.5多专家联合盲区审查。AI匹配18个领域专家按21类盲区清单逐项审查，发现盲区→AI建议方案→成员确认→BA补充→闭环验证。迭代需求额外执行涟漪效应分析。不适用项记录原因后续不复发。这是行业经验驱动的盲区发现机制——解决BA独自分析无法发现行业盲区的问题（如防刷/容灾/对账/迭代影响等）。

> **铁律总数30条，编号BA-01~BA-30全局唯一**。详见 iron-rules-registry.yml。
