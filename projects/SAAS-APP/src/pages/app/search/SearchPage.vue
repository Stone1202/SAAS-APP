    ...
    <div class="sp-input-row">
      <div class="sp-input-box">
        ...
        <input
          ...
          v-model="keyword"
          class="sp-input"
          :placeholder="appConfig.searchHint || '搜索商品、直播、项目'"
      ...
      <span class="sp-cancel" @click="goBack">取消</span>
    </div>
...
const { ucDrawerVisible, ucCards, ucDrawerTitle } = useUseCaseCard('PG-SHP-APP-007', '搜索页');
...
// 热搜词列表（响应式 computed，运营后台修改后自动更新）
const activeHotWords = computed(() =>
  appConfig.hotWordConfigs
...
function doSearch(q: string) {
  ...
  if (hw) {
    ...
    if (csr && csr.status === 'active') {
      ...
      if (csr.jump_type && csr.jump_id) {
        navigateByJumpType(
          csr.jump_type,
          csr.jump_id,
          (csr as any).project_id || '',
          (csr as any).link
        );
        return;
      }
