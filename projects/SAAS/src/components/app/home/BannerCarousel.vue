...
function onClick(banner: any) {
  const jt = banner.jump_type;
  const jid = banner.jump_id;
  const pid = banner.project_id;
  if (jt === 'project') router.push(`/app/project/${jid}`);
  else if (jt === 'product') router.push(`/app/project/${pid}/store/${banner.store_id || ''}`);
  else if (jt === 'live') router.push(`/app/project/${pid}/lives`);
  else if (jt === 'url') router.push(jid);
}
