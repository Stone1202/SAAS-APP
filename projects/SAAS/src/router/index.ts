 ...
 *
 * 单一事实源：PRD §17 路由规划表
 * 设计文档映射：设计文档-内容审查-v1.0.1.md §2.1 页面清单 + §8 交互跳转矩阵
 */
...
// 页面组件（懒加载）
// ============================================
const AuditSwitchPage = () => import('../pages/audit-switch/AuditSwitchPage.vue');
...
const routes: RouteRecordRaw[] = [
  ...
  // ─── PC-租户后台（/tenant/ 前缀）───
  {
    ...
    meta: {
      ...
      system: 'SAAS租户后台',
      fn: [],                         // 入口模拟页，为已有页面提供「更多」菜单入口
      page: 'PG-ENTRY-TENANT-001',
      description: '模拟直播列表入口 → 提供「更多」下拉菜单（查看历史违规列表 / 查看回放）',
    },
  },
  {
    path: '/tenant/live-control/:streamId',
    name: 'LiveControlAudit',
    component: LiveControlAuditPanel,
    meta: {
      terminal: 'pc-tenant',         // PC-租户后台
      system: 'SAAS租户后台',
      fn: ['FN-AUDIT-PC-002', 'FN-AUDIT-PC-003'],  // PRD §17 行2+3+4
      page: 'PG-AUDIT-PC-002',
      description: '直播中控台 → 右侧「内容审查」Tab → 侧滑面板（违规列表+告警统计+处置操作）',
    },
  },
  {
    path: '/tenant/replay/:streamId',
    name: 'ReplayDetailAudit',
    component: ReplayDetailAudit,
    meta: {
      terminal: 'pc-tenant',         // PC-租户后台
      system: 'SAAS租户后台',
      fn: ['FN-AUDIT-PC-004'],       // PRD §17 行5
