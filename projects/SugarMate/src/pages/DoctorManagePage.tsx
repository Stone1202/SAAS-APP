/**
 * PG-DOC-PC-001 医生管理 V3.0.0
 *
 * V3.0.0 改造（与 /apply 医生入驻入口完全对齐）：
 * - 只展示已通过入驻审核 + 上线的医生（ONLINE 状态）
 * - 移除所有审批相关操作 — 由 /onboarding 统一管理
 * - 资质上传：身份证正/反面 + 医师执业证书（与 /apply 一致）
...
const DoctorManagePage: React.FC = () => {
  ...
  const allDoctors = useMemo(
    () => merchants.filter(m => m.role === 'DOCTOR'),
    [merchants]
  );
  ...
  const stats = useMemo(() => ({
    ...
    // 修正为：已上线 / (已上线+已驳回) 百分比
    passRate: (() => {
      const finalized = allDoctors.filter(m =>
        m.lifecycleStatus === 'ONLINE' || m.lifecycleStatus === 'REJECTED'
      );
      if (finalized.length === 0) return 0;
      const passed = allDoctors.filter(m => m.lifecycleStatus === 'ONLINE').length;
      return Math.round((passed / finalized.length) * 100);
    })(),
  ...
  const columns = [
    ...
    { title: '执业医院', dataIndex: 'company', width: 180, ellipsis: true, render: (v: string) => v || '-' },
      ...
      {curMerchant && (
        <div>
          <Descriptions bordered column={2} size="small" style={{ marginBottom: 16 }}>
            ...
            <Descriptions.Item label="角色"><Tag color={ROLE_COLOR.DOCTOR}>医生</Tag></Descriptions.Item>
          ...
          <Descriptions bordered column={2} size="small" style={{ marginBottom: 16 }}>
            ...
            <Descriptions.Item label="执业医院" span={2}>{curMerchant.company || '-'}</Descriptions.Item>
    ...
    <div>
      <Alert
        ...
        description={
          <div style={{ fontSize: 12, lineHeight: 1.8 }}>
            本页仅展示<b>已上线</b>医生（已通过入驻审核）。
            添加医生后，申请将进入 <b>[入驻审核管理]</b> 走完审批流程；
      ...
      >
        <Row gutter={16} style={{ marginBottom: 16 }}>
          <Col span={12}><Card size="small"><Statistic title="已上线医生总数" value={stats.total} prefix={<CheckCircleOutlined style={{ color: 'var(--color-success)' }} />} /></Card></Col>
        ...
        <Space style={{ marginBottom: 16 }} wrap>
          <Input prefix={<SearchOutlined />} placeholder="搜索姓名/手机号/职称/执业医院"
      ...
      {/* 资质上传：身份证正/反面 + 医师执业证书（与 /apply 完全一致） */}
      <MerchantAddDrawer
        open={drawerOpen}
        mode={formMode}
        role={ROLE}
