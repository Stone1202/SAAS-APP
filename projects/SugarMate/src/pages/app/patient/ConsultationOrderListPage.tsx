...
import {
  ClockCircleOutline, CheckCircleOutline,
  MessageOutline, CloseCircleOutline,
...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  PENDING_PRESCRIPTION: { label: '待开方', color: 'purple' },
  PRESCRIPTION_SUBMITTED: { label: '处方已提交', color: 'purple' },
  PRESCRIPTION_SIGNED: { label: '处方已签章', color: 'purple' },
  PRESCRIPTION_APPROVED: { label: '处方已审核', color: 'purple' },
  RX_AWAITING_PATIENT: { label: '待确认处方', color: 'gold' },
  RX_PATIENT_ACCEPTED: { label: '已确认处方', color: 'green' },
  RX_PATIENT_REJECTED: { label: '已拒绝处方', color: 'red' },
  PRESCRIPTION_FLOWING: { label: '处方流转中', color: 'cyan' },
  WAITING_PATIENT_CONFIRM: { label: '待确认完结', color: 'lime' },
  PATIENT_CONFIRMED: { label: '已确认', color: 'green' },
  COMPLETED: { label: '已完成', color: 'green' },
  EVALUATED: { label: '已评价', color: 'green' },
  REFUNDED: { label: '已退款', color: 'red' },
  CANCELLED: { label: '已取消', color: 'default' },
};

const ConsultationOrderListPage: React.FC = () => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (activeKey === 'active') return orders.filter(o =>
      ['PENDING_ACCEPT', 'ACCEPTED', 'IN_CONSULT', 'PENDING_PRESCRIPTION',
       ...
       'PRESCRIPTION_FLOWING', 'WAITING_PATIENT_CONFIRM'].includes(o.status)
    );
    if (activeKey === 'completed') return orders.filter(o =>
      ['PATIENT_CONFIRMED', 'COMPLETED', 'EVALUATED'].includes(o.status)
    );
    return orders;
  }, [orders, activeKey]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5' }}>
      <NavBar onBack={() => navigate(-1)}>问诊记录</NavBar>
      ...
      <div style={{ padding: '8px 12px' }}>
        {filteredOrders.length === 0 ? (
          <Empty description="暂无问诊记录" style={{ paddingTop: 60 }} />
        ) : (
          <List>
            {filteredOrders.map((order) => {
              ...
              return (
                <List.Item
                  key={order.id}
                  clickable
                  onClick={() => navigate(`/app/consultation/chat/${order.id}`)}
                  extra={
...
export default ConsultationOrderListPage;
